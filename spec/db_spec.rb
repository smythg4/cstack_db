describe 'database' do
  ...
  def run_script(commands)
    raw_output = nil
    IO.popen("./target/debug/cstack_db test.db", "r+") do |pipe|
  ...
  it 'prints error message when table is full' do
    ...
    expect(result.last(2)).to match_array([
      "db > Executed.",
      "db > Tried to fetch page number (100) out of bounds. Max: 100",
    ])
  end
  ...
  it 'keeps data after clsoing connection' do
    result1 = run_script([
      "insert 1 user1 person1@example.com",
      ".exit",
    ])
  ...
  it 'selects all rows correctly from a scrambled-insert multi-level tree' do
    script = [
    ...
    rows = (1..30).map { |i| "(#{i}, user#{i}, person#{i}@example.com)" }
    expected = ["db > #{rows[0]}"] + rows[1..] + ["Executed.", "db > "]

    expect(result[30...result.length]).to eq(expected)
  end
